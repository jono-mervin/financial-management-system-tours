...
$id = (int)($_GET['id'] ?? 0);
$stmt = $pdo->prepare("SELECT i.*, v.vendor_name, v.vendor_type, v.email, v.phone FROM ap_invoices i JOIN vendors v ON v.vendor_id = i.vendor_id WHERE i.ap_invoice_id = ?");
$stmt->execute([$id]);
$inv = $stmt->fetch();

if (!$inv) { flash('error', 'Invoice not found.'); header('Location: invoices.php'); exit; }

$breadcrumb = ['Accounts Payable', 'Invoices', $inv['invoice_no']];
require_once __DIR__ . '/../../includes/header.php';

$disp = invoice_display_status($inv['status'], $inv['due_date']);
$balance = $inv['amount'] - $inv['amount_paid'];

$disbursements = $pdo->prepare("SELECT * FROM disbursements WHERE ap_invoice_id = ? ORDER BY payment_date");
$disbursements->execute([$id]);
...
<div class="flex items-center justify-between mb-5 -mt-2">
    ...
    <div class="flex items-center gap-3 no-print">
        ...
        <a href="../disbursement/disbursements.php?ap_invoice_id=<?= $id ?>" class="bg-primary hover:bg-primary-light text-white text-sm font-medium px-4 py-2 rounded-xl">Record Disbursement</a>
        <?php endif; ?>
        <?php if ($inv['status'] === 'Unpaid' && $inv['amount_paid'] == 0): ?>
        <form method="post" action="invoice-save.php" data-confirm="Cancel this invoice? Its journal entry will be voided.">
            <input type="hidden" name="action" value="cancel">
            <input type="hidden" name="ap_invoice_id" value="<?= $id ?>">
...
<div class="bg-white rounded-2xl border border-black/5 p-8 mb-6">
    <div class="flex items-start justify-between pb-6 mb-6 border-b border-black/5">
        <div>
            <p class="text-xs font-semibold text-ink/40 uppercase tracking-wide mb-1">AP Invoice</p>
    ...
    <div class="grid grid-cols-3 gap-6 mb-8 text-sm">
        <div>
            ...
            <p class="text-ink font-medium"><?= e($inv['vendor_name']) ?></p>
            <p class="text-ink/40 text-xs mt-0.5"><?= e($inv['vendor_type']) ?></p>
        </div>
        <div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-2">
        ...
        <div class="pass-card p-4">
            <p class="text-xs text-ink/40 uppercase tracking-wide">Paid</p>
            <p class="font-mono text-xl font-bold text-emerald-600 mt-1"><?= money((float)$inv['amount_paid']) ?></p>
        </div>
        <div class="pass-card p-4">
...
<div class="bg-white rounded-2xl border border-black/5 overflow-hidden">
    <div class="px-6 py-4 border-b border-black/5"><h3 class="font-display font-semibold text-ink">Disbursement History</h3></div>
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-black/5">
            ...
            <tr>
                <td class="px-6 py-3 font-mono text-xs text-ink/70"><?= e($d['disbursement_no']) ?></td>
                <td class="px-3 py-3 text-ink/60"><?= date('M j, Y', strtotime($d['payment_date'])) ?></td>
                <td class="px-3 py-3 text-ink/60"><?= e($d['payment_method']) ?></td>
                <td class="px-6 py-3 text-right font-mono"><?= money((float)$d['amount']) ?></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
